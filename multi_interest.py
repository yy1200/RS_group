...
class SelfAttention(nn.Module):
    def __init__(self, d_hid, dropout=0.):
        ...
        self.scorer2 = nn.Linear(1, 100)
        self.dropout = nn.Dropout(dropout) #根據白努力隨機將值變為0，防止overfitting
        self.reset_parameters(d_hid)
    def reset_parameters(self,d_hid):
        stdv = 1. / math.sqrt(d_hid)
        for weight in self.parameters():
            weight.data.uniform_(-stdv, stdv)
    def forward(self, input_seq,seqemb=None,s=None):
        batch_size, seq_len, feature_dim = input_seq.size()
        input_seq = self.dropout(input_seq)
#         print(seqemb.size())
        if seqemb!=None:
            size=input_seq.shape[1]
            anchor=seqemb.repeat(1,size,1) #[2, 100]
            seq=torch.cat((input_seq,anchor),2) #[2, 200]
            scores = self.scorer(seq)
            scores = F.sigmoid(scores)
            scores = self.scorer2(scores)
#             print(scores.size())
#             print(s.size())
            scores = scores+s
        else:
            scores = self.scorer(input_seq) #copy tensor，元素以連續的方式存在內存中
            scores = F.sigmoid(scores)
            scores = self.scorer2(scores)
#             print(scores.size())
        scores = F.softmax(scores)
        context = scores.expand_as(input_seq).mul(input_seq).sum(1)
...
class GraphEncoder(Module):
    def __init__(self,graph,device, entity, emb_size, kg, embeddings=None, fix_emb=True, seq='rnn', gcn=True, hidden_size=100, layers=1, rnn_layer=1,u=None,v=None,f=None,g=None):
        super(GraphEncoder, self).__init__()

        self.eps = torch.nn.Parameter(torch.Tensor([0]))
        # self.eps = 0.0
        self.relu = nn.LeakyReLU(0.2, inplace=True)
        rel_names=['member_of','group_interact','group_like','interact','friends','like','belong_to']
        ...
        self.multi_head_self_attention = nn.ModuleList([SelfAttention(self.hidden_size*2, 0.3) for _ in range(self.num_pers)])
    def forward(self, b_state):
        ...
        h0={'group': self.embedding(torch.arange(0, self.group_num).long().to(self.device)),
            'user': self.embedding(torch.arange(self.group_num, self.group_num+self.user_num).long().to(self.device)),
            'item': self.embedding(torch.arange(self.group_num+self.user_num, self.group_num+self.user_num+self.item_num).long().to(self.device)),
            'attribute': self.embedding(torch.arange(self.group_num+self.user_num+self.item_num, self.PADDING_ID+1).long().to(self.device))
           }
        h1 = self.conv1(self.G, h0)
        h1 = {k: self.relu((v)) for k, v in h1.items()}
        
        gnn_embedding=torch.cat((h1['group'],h1['user']),dim=0)
        gnn_embedding=torch.cat((gnn_embedding,h1['item']),dim=0)
