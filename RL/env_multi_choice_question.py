...
class MultiChoiceRecommendEnv(object):
    def __init__(self, kg, data_name, embed, seed=1, max_turn=15, cand_num=10, cand_item_num=10, attr_num=20, mode='train', ask_num=1, entropy_way='weight entropy', fm_epoch=0,choice_num=2,fea_score="other"): #entropy
        ...
        self.kg = kg
        self.group_length = len(self.kg.G['group'].keys())
        self.user_length = len(self.kg.G['user'].keys())
        self.item_length = len(self.kg.G['item'].keys())
        self.large_feature_length = 7
        self.feature_length = len(self.kg.G['feature'].keys())
        ...
        if embeds is not None:
            ...
            self.ui_embeds = nn.Embedding(self.group_length+self.user_length+self.item_length, 64).weight.data.numpy()
    ...
    def reset(self, embed=None):
        if embed is not None:
            self.ui_embeds = embed[:self.group_length+self.user_length+self.item_length]
            self.feature_emb = embed[self.group_length+self.user_length+self.item_length:]
        #init  user_id  item_id  cur_step   cur_node_set
        self.cur_conver_step = 0   #reset cur_conversation step
        self.cur_node_set = []
        self.rej_item=[]
        if self.mode == 'train':
            groups = list(self.kg.G['group'].keys())
            self.user_id = np.random.choice(groups)
            self.members_id = list(self.kg.G['group'][self.user_id]['member_of'])
            while len(self.u_multi[str(self.user_id)]) == 0:
                self.user_id = np.random.choice(groups)
...
#         enablePrint()
        ...
        if len(attrs)==0:
            attrs = set(attrs)|set(attrs2)
        attrs=list(attrs)
            
        user_like_random_fea = random.choice(attrs)
        self.user_acc_feature[user_like_random_fea] = 1
#         self.user_acc_feature.append(user_like_random_fea) #update user acc_fea
    ...
    def _get_state(self):
        if self.data_name in ['YELP_STAR', 'AMAZON', 'ML1M']:
            self_cand_items = self.cand_items[:5000]
            set_cand_items = set(self_cand_items)
        else:
            self_cand_items = self.cand_items        
        user = [self.user_id]
        cur_node = [x + self.group_length + self.user_length + self.item_length for x in self.cur_node_set] 
        cand_items = [x + self.group_length + self.user_length for x in self_cand_items]
        reachable_feature = [x + self.group_length + self.user_length + self.item_length for x in self.reachable_feature]
        ...
        state = {'cur_node': cur_node,
                 'neighbors': neighbors,
                 'adj': adj,
                 'rej_feature': list(self.user_rej_feature.keys()),
    ...
    def step(self, action, sorted_actions, embed=None):  
        ...
        if self.cur_conver_step == self.max_turn:
        ...
        elif action >= self.group_length + self.user_length + self.item_length:   #ask feature
            score={}
            max_score_id=None
            max_score=-999
            large_small={}
            for i in range(len(sorted_actions)):
                ...
                if act < self.group_length + self.user_length + self.item_length:
                    continue
        ...
        if len(self.reachable_feature) != 0:  # if reachable_feature == 0 :cand_item= 1
            ...
            [self.reachable_feature.remove(v) for v in max_fea_id]
            [self.reachable_feature.insert(0, v) for v in max_fea_id[::-1]]
#             random.shuffle(self.reachable_feature)

        self.cur_conver_step += 1
        return self._get_state(), self._get_cand(), self._get_action_space(), reward, done, recom_items, is_rec
    ...
    def _feature_score(self):
        ...
        if self.fea_score=="entropy":
            for feature_id in self.reachable_feature:
                ...
                prefer_embed = self.feature_emb[list(self.user_acc_feature.keys()), :]  #np.array (x*64)
            ...
            for feature_id in self.reachable_feature:
                ...
                acc_fea_list = list(self.user_acc_feature.keys())
                prefer_embed = self.feature_emb[acc_fea_list, :]  #np.array (x*64)
                rej_fea_list = list(self.user_rej_feature.keys())
                rej_embed = self.feature_emb[rej_fea_list, :]  #np.array (x*64)
                for i in range(len(self.user_acc_feature)):
                    score += np.inner(prefer_embed[i]*self.user_acc_feature[acc_fea_list[i]], fea_embed)
                for i in range(len(self.user_rej_feature)):
                    score -= self.sigmoid([np.inner(rej_embed[i]*self.user_rej_feature[rej_fea_list[i]], fea_embed)])
                reach_fea_score.append(score)
    ...
    def _item_score(self):
        ...
        for item_id in self.cand_items:
            item_embed = self.ui_embeds[self.group_length + self.user_length + item_id]
            score = 0
    ...
    def _item_score_pop(self):
        df = pd.read_csv('data/ml1m/Graph_generate_data/pop_list.csv')
        cand_item_score = []
        for item_id in self.cand_items:
            score = df[str(item_id)].item()
            cand_item_score.append(score)
        return cand_item_score
    
    def _item_score_random(self):
        cand_item_score = []
        for item_id in self.cand_items:
    ...
    def _ask_update(self, asked_features):
        ...
        for asked_feature in asked_features:
            ...
            for m in self.members_id:
                if asked_feature in self.kg.G['user'][m]['like']:
                    accept_num += 1
            accept_degree = accept_num/len(self.members_id)
                    
            if asked_feature in self.feature_groundtrue or accept_degree>=0.5:
                acc_rej = True
                self.user_acc_feature[asked_feature] = accept_degree
#                 self.user_acc_feature.append(asked_feature)
                ...
                self.user_rej_feature[asked_feature] = 1-accept_degree
#                 self.user_rej_feature.append(asked_feature)
                reward += self.reward_dict['ask_fail']
                  #update conver_his
        print('user_acc_feature:', self.user_acc_feature)
        print('user_rej_feature:', self.user_rej_feature)
...
#             accept_num = 0
#             for m in self.members_id:
#                 if fea in self.kg.G['user'][m]['like']:
#                     accept_num += 1
            if fea in self.feature_groundtrue:
                print('=== ask acc {}: update cand_items'.format(fea))
                feature_items = self.kg.G['feature'][fea]['belong_to'] # item
        ...
        # random
#         cand_item_score = self._item_score_random()
        # pop list
#         cand_item_score = self._item_score_pop()
        #記得調422行 
    ...
    def _map_to_all_id(self, x_list, old_type):
        if old_type == 'item':
            return [x + self.group_length + self.user_length for x in x_list]
        elif old_type == 'feature':
            return [x + self.group_length + self.user_length + self.item_length for x in x_list]
        else:
            return x_list

    def _map_to_old_id(self, x):
        if x >= self.group_length + self.user_length + self.item_length:
            x -= (self.group_length + self.user_length + self.item_length)
        elif x >= self.group_length + self.user_length:
            x -= self.group_length + self.user_length
        return x
