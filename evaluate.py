...
from RL.RL_evaluate import dqn_evaluate, cal_NDCG, cal_P_R
...
def evaluate(args, kg, filename):
    SR5_best, SR10_best, SR15_best, AvgT_best, Rank_best, NDCG_best, P_best, R_best = 0.,0.,0.,15.,0.,0.,0.,0.
    ...
    G=get_graph(test_env.user_length,test_env.item_length,test_env.feature_length,test_env.group_length,args.data_name)
    gcn_net = GraphEncoder(graph=G,device=args.device, entity=embed.size(0), emb_size=embed.size(1), kg=kg, embeddings=embed, \
        fix_emb=args.fix_emb, seq=args.seq, gcn=args.gcn, hidden_size=args.hidden,u=test_env.user_length,v=test_env.item_length,f=test_env.feature_length,g=test_env.group_length).to(args.device)
    ...
    SR5_mean, SR10_mean, SR15_mean, AvgT_mean, Rank_mean, NDCG_mean, P_mean, R_mean = dqn_evaluate(args, kg, agent, filename, 0)
    ...
    print("best!!!!!!!!!SR5:{}, SR10:{}, SR15:{}, AvgT:{}, Rank:{}, NDCG:{}, Precision:{}, Recall:{}!!!!".format(SR5_best, SR10_best, SR15_best, AvgT_best, Rank_best, NDCG_best, P_best, R_best))
...
def main():
    ...
    parser.add_argument('--data_name', type=str, default=ML1M, choices=[ MOVIE,LAST_FM_STAR, YELP_STAR,BOOK,AMAZON],
    ...
    parser.add_argument('--load_rl_epoch', type=int, default=10, help='the epoch of loading RL model') #revise
    ...
    parser.add_argument('--max_steps', type=int, default=30, help='max training steps')
